...
func SetValueAndSave(name, key, value string) (err error) {
	...
	GetRemoteConfig().GetRemote(name).SetString(key, value)
...
func FileGetFresh(section, key string) (value string, err error) {
	...
	return GetRemoteConfig().GetRemote(section).GetString(key), nil
}

// ShowRemotes shows an overview of the config file
func ShowRemotes() {
	remotes := GetRemoteConfig().ListRemotes()
	if len(remotes) == 0 {
		return
	}
...
func ShowRemote(name string) {
	...
	for _, key := range GetRemoteConfig().GetRemote(name).Keys() {
		isPassword := false
...
func ChooseOption(o *fs.Option, name string) string {
	var subProvider = GetRemoteConfig().GetRemote(name).GetString(fs.ConfigProvider)
	fmt.Println(o.Help)
	if o.IsPassword {
...
func CreateRemote(name string, provider string, keyValues rc.Params) error {
	...
	GetRemoteConfig().DeleteRemote(name)
	// Set the type
	GetRemoteConfig().GetRemote(name).SetString("type", provider)
...
func copyRemote(name string) string {
	...
	oldRemote := GetRemoteConfig().GetRemote(name)
	newRemote := GetRemoteConfig().CreateRemote(newName)
	// Copy the keys
	for _, key := range oldRemote.Keys() {
		newRemote.SetString(key, oldRemote.GetString(key))
	}
	return newName
}
...
func EditConfig() {
	for {
		haveRemotes := len(GetRemoteConfig().ListRemotes()) != 0
...
func Authorize(args []string, noAutoBrowser bool) {
	...
	if noAutoBrowser {
		GetRemoteConfig().GetRemote(name).SetString(ConfigAuthNoBrowser, "true")
	}
...
func FileGet(section, key string, defaultVal ...string) string {
	...
	value := GetRemoteConfig().GetRemote(section).GetString(key)
	if len(value) == 0 && len(defaultVal) > 0 {
		return defaultVal[0]
	}
...
func FileSet(section, key, value string) {
	if value != "" {
		GetRemoteConfig().GetRemote(section).SetString(key, value)
	} else {
...
func FileDeleteKey(section, key string) bool {
	return GetRemoteConfig().GetRemote(section).Delete(key)
}
...
func FileSections() []string {
	sections := GetRemoteConfig().ListRemotes()
...
func DumpRcBlob() (dump rc.Params) {
	dump = rc.Params{}
	for _, name := range GetRemoteConfig().ListRemotes() {
